...
typedef int (* ZqCallBack)(int,int,void *);
...
int zqreactor_addlistener(struct zqreactor *reactor,int sockfd,ZqCallBack acceptor){
    if(reactor == NULL) return -1;
...
int zqreactor_run(struct zqreactor *reactor,threadpool_t *pool) {
    if (reactor == NULL) return -1;
    ...
    while(1){
        ...
        for(i = 0;i < nready;i++){
            ...
            if((zqev->events & EPOLLIN) && (events[i].events & EPOLLIN)){
                Event event= Event(i,zqev->events,zqev->arg,zqev->callback);
                threadpool_add(pool,event);
